...
Each row reconstructs a Registry-backed scheduled task, with task identity, author, triggers, actions, command and arguments, registration or run metadata, exit code, and security information where available. Use it to hunt persistence and correlate task actions with binaries or accounts. Registration does not prove execution, and last-run fields may be absent or stale.
...
|---|---|---|
| `guid` | String | unique identifier (GUID) of the scheduled task |
| `uri` | String | URI associated with the scheduled task |
...
| `boot` | Object |  |
| `boot.mtime` | DateTime | modification timestamp of the boot sub‑key for the task |
| `logon` | Object |  |
| `logon.mtime` | DateTime | modification timestamp of the logon sub‑key for the task |
| `maintenance` | Object |  |
| `maintenance.mtime` | DateTime | modification timestamp of the maintenance sub‑key for the task |
| `plain` | Object |  |
| `plain.mtime` | DateTime | modification timestamp of the plain sub‑key for the task |
| `tree` | Object |  |
| `tree.mtime` | DateTime | modification timestamp of the tree sub‑key for the task |
| `creation_date` | String | timestamp when the scheduled task was initially created |
| `last_run_launch_date` | DateTime | timestamp of the most recent task launch |
| `last_run_exit_code` | Int | exit code returned by the latest execution of the task |
| `last_run_exit_date` | DateTime | timestamp when the latest task execution completed |
| `registration_date_local` | DateTime | date the task was registered |
