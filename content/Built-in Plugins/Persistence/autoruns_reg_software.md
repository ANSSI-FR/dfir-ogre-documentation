...
Each row is a machine-wide auto-start value found in a known SOFTWARE-hive persistence location, with persistence type, key and value, command data, LastWrite time, owner, and permissions. Use it to identify configured persistence and correlate commands with files or accounts. Registry configuration does not prove the entry executed, and LastWrite applies to the key rather than an individual value.
